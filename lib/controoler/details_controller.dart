...
class DetailsController extends GetxController {
  ...
  // ========== Fetch INVENTAIRD  ==========
  Future<void> fetchInventaired() async {
    try {
      ...
      if (response.statusCode == 200) {
        List<dynamic> body = response.body['INVENTAIRD'];
        inventaireDetaileList = body.map((dynamic item) => InventairedModel.fromJson(item)).toList();
        for (var element in inventaireDetaileList) {
          if (articles.any((article) => article.artno == element.iNDART)) {
            element.iNDARTNom = articles.firstWhere((article) => article.artno == element.iNDART).artnom;
          }
        }
        //,
      } else {
